...
local _handlers, handlers = pcall(require, "configuration.plugins.lsp.handlers")
...
if not _handlers then
	vim.notify("handlers script not loaded")
	return
end
...
local java_cmd = "java" -- done like this because its on the PATH
local mason_registry = require("mason-registry")

local bundles = {
	mason_registry.get_package("java-debug-adapter"):get_install_path()
		.. "/extension/server/com.microsoft.java.debug.plugin-0.47.0.jar",
}
vim.list_extend(
	bundles,
	vim.split(
...
local config = {
	...
	root_dir = jdtls.setup.find_root({ ".metadata", "pom.xml", ".git", "gradlew", "mvnw", "build.gradle" }),
